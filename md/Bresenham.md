...
## 고쳐야 할 점
Bresenham을 구현해 직선을 그리는 과정에서, 계속 원하는 대로 선이 안 그려지는 문제가 있었다. 변환된 모든 좌표를 눈으로 보며 검사해 본 결과, 직선의 기울기에 따른 분기가 충분히 나눠지지 않아 원하는 대로 선이 안 그려지고 있었다. 그래서 확실하게 따지기 위해 모든 분기를 나눠서 처리했는데, 이 과정에서 비슷한 작업을 하는 중복된 코드가 많이 나오게 됐다. 좀 더 생각을 잘 해서 코드를 짰다면 아마 절반 정도로 줄일 수 있었을 것 같아 아쉬운 점이 많았다. 
